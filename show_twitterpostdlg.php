...
#############################################################################
  ...
  if(!isset($SendStatId)) {
    $_GET["action"] = "show_twitterpostdlg.php";
    _LOQCJ($CampaignId);
    if(!isset($_POST["SendStatId"]))
       exit;
       else
  ...
  if(isset($_POST["SubmitBtn"])) {
    if(empty($_POST["twittertweet"]))
      $errors[] = "twittertweet";
    if(count($errors) == 0){
      //$_SESSION["twittertext"] = $_POST["twittertweet"];
      include("twittersend.php");
      exit;
    }
  }

  // Template
  $_QJCJi = GetMainTemplate(true, $UserType, $Username, true, $resourcestrings[$INTERFACE_LANGUAGE]["000740"], $_I0600, 'browsecampaigns', 'twitter_post.htm');
  ...
  if(empty($_POST["twittertweet"])) {
    ...
    $_POST["twittertweet"] = $_Q6Q1C["MailSubject"]."\r\n".$_6Olt8;
  }
  ...
  function _LOQCJ($_I6lOO){
    ...
    $_IiQl1 = $_Q6jOo;
    $_QJlJ0 = "SELECT Name, CurrentSendTableName FROM $_IiQl1 WHERE id=$_I6lOO";
    $_Q60l1 = mysql_query($_QJlJ0);
    _OAL8F($_QJlJ0);
    $_Q6Q1C=mysql_fetch_array($_Q60l1);
    $_j0fti = $_Q6Q1C["CurrentSendTableName"];
    ...
    if(mysql_num_rows($_Q60l1) != 1) {
      ...
      $_QJCJi = GetMainTemplate(true, $UserType, $Username, true, $resourcestrings[$INTERFACE_LANGUAGE]["000740"], $_I0600, 'DISABLED', 'twitter_post_sententry_select.htm');
