 ...
 //   var $soap_defencoding = 'ISO-8859-1';
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
       ...
       $array_typename = 'xsd:anyType'; // M.B. changed to xsd:anyType
						} else {
							// if type is prefixed, create type prefix
...
class wsdl extends nusoap_base {
  ...
  $PHP_SELF = htmlentities($PHP_SELF);
		...
		<div class=content>
          ...
          $donnie = htmlentities($donnie);

						if($donnie == 'input' || $donnie == 'output'){ // show input/output data
						    $b .= "<font color='white'>".ucfirst($donnie).':</font><br>';
						    foreach($marie as $captain => $tenille){ // loop through data
          $captain = htmlentities($captain);

								if($captain == 'parts'){ // loop thru parts
								    $b .= "&nbsp;&nbsp;$captain:<br>";
					                //if(is_array($tenille)){
								    	foreach($tenille as $joanie => $chachi){
                $joanie = htmlentities($joanie);
                $chachi = htmlentities($chachi);
     											$b .= "&nbsp;&nbsp;&nbsp;&nbsp;$joanie: $chachi<br>";
								    	}
					        		//}
								} else {
...
class nusoap_client extends nusoap_base  {
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->appendDebug($this->varDump($headers));
    	if (!isset($headers['content-type'])) {
			$this->setError('Response not of type text/xml (no content-type header)'. ' ' .$data);
			return false;
    	}
		if (!strstr($headers['content-type'], 'text/xml')) {
			$this->setError('Response not of type text/xml: ' . $headers['content-type']. ' ' .$data);
			return false;
		}
		if (strpos($headers['content-type'], '=')) {
			$enc = str_replace('"', '', substr(strstr($headers["content-type"], '='), 1));
