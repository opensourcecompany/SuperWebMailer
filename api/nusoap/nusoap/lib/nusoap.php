  ...
  $PHP_SELF = htmlentities($PHP_SELF);
		...
		<div class=content>
			...
			<div class=nav>
				...
				Click on an operation name to view it&#39s details.</p>
				<ul>';
          ...
          $donnie = htmlentities($donnie, ENT_COMPAT, $this->soap_defencoding);

						if($donnie == 'input' || $donnie == 'output'){ // show input/output data
						    $b .= "<font color='white'>".ucfirst($donnie).':</font><br>';
						    foreach($marie as $captain => $tenille){ // loop through data
          $captain = htmlentities($captain, ENT_COMPAT, $this->soap_defencoding);

								if($captain == 'parts'){ // loop thru parts
								    $b .= "&nbsp;&nbsp;$captain:<br>";
					                //if(is_array($tenille)){
								    	foreach($tenille as $joanie => $chachi){
                $joanie = htmlentities($joanie, ENT_COMPAT, $this->soap_defencoding);
                $chachi = htmlentities($chachi, ENT_COMPAT, $this->soap_defencoding);
     											$b .= "&nbsp;&nbsp;&nbsp;&nbsp;$joanie: $chachi<br>";
								    	}
					        		//}
								} else {
						    ...
						    $b .= "<font color='white'>".ucfirst($donnie).":</font> ".htmlentities(utf8_encode($marie), ENT_COMPAT, $this->soap_defencoding)."<br>";
						}
				    }
					$b .= '</div>';
				}
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					if(isset($opParts['documentation']) && $opParts['documentation'] != '') {
						$portType_xml .= "\n" . '    <documentation>' . htmlspecialchars( utf8_encode($opParts['documentation']), ENT_COMPAT, $this->soap_defencoding) . '</documentation>';
					}
