...
#############################################################################
  ...
  if ( (isset($_POST["action"]) && $_POST["action"] == "local") ||
       (isset($_GET["action"]) && $_GET["action"] == "local")
         ...
         if (! isset($_POST["OneMailingListId"]) && ! isset($_GET["OneMailingListId"]) ) {
           $_Itfj8 = $resourcestrings[$INTERFACE_LANGUAGE]["001140"];
  ...
  if(isset($_POST["step"]) && $_POST["step"] == 3) {
   if(!isset($_POST["fields"])) {
     ...
     $_POST["step"] = 2;
     $_Iio6I[] = "fields[Domainname]";
   } else {
     if( !isset($_POST["fields"]["Domainname"]) || $_POST["fields"]["Domainname"] == "-1" ) {
  ...
  if($_Ii6CO)
   $_IiOfO = $resourcestrings[$INTERFACE_LANGUAGE]["001139"];
   else
   $_IiOfO = $_Ii8Q6." - ".$resourcestrings[$INTERFACE_LANGUAGE]["001140"];

  if(!isset($_POST["step"])) {

    $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, "", 'domainblocklistmemberimport', 'blocklistimport1_snipped.htm');
    $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
    ...
    if(!(strpos($_It18j, "G") === false))
       $_ItQIo = $_It18j * 1024 * 1024 * 1024;
       else
       if(!(strpos($_It18j, "M") === false))
          $_ItQIo = $_It18j * 1024 * 1024;
          else
    ...
    if( isset($_POST["OneMailingListId"]) && $_POST["OneMailingListId"] > 0 ) {
       ...
       $_QLJfI = str_replace('domainblocklistmemberimport.php', 'domainblocklistmemberimport.php?action=local&OneMailingListId='.$_POST["OneMailingListId"], $_QLJfI);
    ...
    exit;
      ...
      if( isset( $_FILES['file1'] ) && $_FILES['file1']['tmp_name'] != "" && $_FILES['file1']['name'] != "" ) {
        ...
        if (move_uploaded_file($_FILES['file1']['tmp_name'], $_ItL8f.$_FILES['file1']['name'])){
           ...
           $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, sprintf($resourcestrings[$INTERFACE_LANGUAGE]["000055"], $_FILES['file1']['tmp_name'], $_ItL8f.$_FILES['file1']['name'] ), 'domainblocklistmemberimport', 'blocklistimport1_snipped.htm');
           $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
           $_QLJfI = str_replace("browseblmembers.php", "browsedomainblmembers.php", $_QLJfI);
           $_QLJfI = str_replace('/userfiles/import', $_ItL8f, $_QLJfI);
           $_It18j = ini_get("upload_max_filesize");
      ...
      $_IiC0o = _JOO1L("DomainBlockListImportOptions");
      $_IiCft = $_POST;
      unset($_IiCft["step"]);
      unset($_IiCft["action"]);
      if( $_IiC0o != "" ) {
    ...
    $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, $_Itfj8, 'domainblocklistmemberimport', 'blocklistimport3_snipped.htm');
    $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
    $_QLJfI = str_replace("browseblmembers.php", "browsedomainblmembers.php", $_QLJfI);
    ...
    $_Ql0fO = str_replace('<!--FIELD'.$_Qli6J.'-->', $resourcestrings[$INTERFACE_LANGUAGE]["DomainName"], $_Ql0fO);
    $_Ql0fO = str_replace('<!--VALUE'.$_Qli6J.'-->', '<select name="fields['."Domainname".']" size="1">'.$_I0Clj.'</select>', $_Ql0fO);
    ...
    $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, "", 'domainblocklistmemberimport', 'blocklistimport4_snipped.htm');
    $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
    $_QLJfI = str_replace("browseblmembers.php", "browsedomainblmembers.php", $_QLJfI);
    ...
    $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, "", 'domainblocklistmemberimport', 'blocklistimport5_snipped.htm');
    $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
    $_QLJfI = str_replace("browseblmembers.php", "browsedomainblmembers.php", $_QLJfI);
    ...
    if(isset($_POST["fileposition"])) {
        ...
        if($_Iiioi == -1 || feof($_IJljf) || ftell($_IJljf) >= $_If61l["size"] ) {
          ...
          $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, $_Itfj8, 'domainblocklistmemberimport', 'blocklistimport6_snipped.htm');
          $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
          $_QLJfI = str_replace("browseblmembers.php", "browsedomainblmembers.php", $_QLJfI);
    ...
    if( isset($_POST["OneMailingListId"]) && $_POST["OneMailingListId"] > 0 ) {
       $_IilfC = str_replace('browsedomainblmembers.php"', 'browsedomainblmembers.php?action=local&OneMailingListId='.$_POST["OneMailingListId"].'"', $_IilfC);
       $_IilfC = str_replace('domainblocklistmemberimport.php', 'domainblocklistmemberimport.php?action=local&OneMailingListId='.$_POST["OneMailingListId"], $_IilfC);
       $_IilfC = str_replace('exportdomainblocklist.php', 'exportdomainblocklist.php?action=local&OneMailingListId='.$_POST["OneMailingListId"], $_IilfC);
    }
    ...
    for($_Qli6J=$_Iil6i; ($_Qli6J<$_Iil6i + intval($_POST["ImportLines"])) && !feof($_IJljf); $_Qli6J++) {
      $_IiiJf++;
      $_IiLOj = fgets($_IJljf, 4096);
      // UTF8?
      if ( !( isset($_POST["IsUTF8"]) && $_POST["IsUTF8"] != "") ) {
      ...
      foreach($_IOJoI as $_IOLil => $_Iiloo) {
        ...
        if(isset($_IOJoI[$_IOLil]) && $_IOCjL != -1) {
          ...
          # no empty domains
          if($_IOLil == "Domainname") {
            $_IOCjL = _L86JE( trim($_IOCjL) );
            if($_IOCjL == "") {
              $_QLfol = "";
              break;
            }

            if(strpos($_IOCjL, '@') !== false) {
              $_IOCjL = substr($_IOCjL, strpos($_IOCjL, '@') + 1);
            }

            if( !_L8JLR('info@'.$_IOCjL) ) {
              $_QLfol = ""; // invalid
              break;
            }
          }
    ...
    $_Iil6i += intval($_POST["ImportLines"]);
    $_Iiioi = ftell($_IJljf);
    if(feof($_IJljf))
      $_Iiioi = -1;
  ...
  function _L0FRP($errors, $_Io0OJ, $_Itfj8 = "") {
    ...
    $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, $_Itfj8, 'domainblocklistmemberimport', 'blocklistimport2_snipped.htm');
    $_QLJfI = str_replace("blocklistmemberimport.php", "domainblocklistmemberimport.php", $_QLJfI);
    $_QLJfI = str_replace("browseblmembers.php", "browsedomainblmembers.php", $_QLJfI);
    ...
    while (false !== ($_QlCtl = readdir($_IJljf))) {
      if (!is_dir($_ItL8f.$_QlCtl) && $_QlCtl != "." && $_QlCtl != ".." && $_QlCtl != "index.php") {
        $_IO6iJ .= '<option value="'.$_QlCtl.'">'.$_QlCtl.'</option>'.$_QLl1Q;
      }
    }
    closedir($_IJljf);
