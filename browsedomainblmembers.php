...
#############################################################################
  ...
  if ( (isset($_POST["Action"]) && $_POST["Action"] == "local") || (isset($_POST["action"]) && $_POST["action"] == "local") ||
       (isset($_GET["Action"]) && $_GET["Action"] == "local") || (isset($_GET["action"]) && $_GET["action"] == "local")
         ...
         // get local blocklist
         $_QLfol = "SELECT LocalDomainBlocklistTableName, Name FROM $_QL88I WHERE id=".$_POST["OneMailingListId"];
        ...
        if($_POST["BlocklistActions"] == "RemoveMembers") {
          ...
          if ( isset($_POST["OneMemberId"]) && $_POST["OneMemberId"] > 0 )
              ...
              if ( isset($_POST["MemberIDs"]) )
                $_IlOlj = array_merge($_IlOlj, $_POST["MemberIDs"]);
          ...
          // show now the list
          if(count($_IQ0Cj) > 0)
            $_Itfj8 = $resourcestrings[$INTERFACE_LANGUAGE]["001133"].join("<br />", $_IQ0Cj);
          else
            $_Itfj8 = $resourcestrings[$INTERFACE_LANGUAGE]["001132"];
        }
    }
  ...
  if ( (count($_POST) <= 1) || (isset($_POST["MailingListSelectForm"])) || ( isset($_POST["OneBlocklistAction"]) && $_POST["OneBlocklistAction"] == "BrowseMembers") ) {
    ...
    $_j1ooo = _JOO1L("BrowseDomainBlocklistsFilter");

    if( $_j1ooo != "") {
      $_I016j = @unserialize($_j1ooo);
      if($_I016j !== false)
  ...
  // Template
  if($_Ii6CO)
   $_IiOfO = $resourcestrings[$INTERFACE_LANGUAGE]["001130"].$resourcestrings[$INTERFACE_LANGUAGE]["EntryCount"];
   else
   $_IiOfO = $_Ii8Q6." - ".$resourcestrings[$INTERFACE_LANGUAGE]["001131"].$resourcestrings[$INTERFACE_LANGUAGE]["EntryCount"];

  $_QLJfI = GetMainTemplate(true, $UserType, $Username, true, $_IiOfO, $_Itfj8, 'browsedomainblmembers', 'browse_domain_blocklist_members_snipped.htm');

  // hold hidden the listname
  if(isset($_POST["OneMailingListId"]))
    $_QLJfI = str_replace('name="OneMailingListId"', 'name="OneMailingListId" value="'.intval($_POST["OneMailingListId"]).'"', $_QLJfI);
  ...
  if($OwnerUserId != 0) {
    ...
    if(!$_Ii6CO) {
      if(!$_QLJJ6["PrivilegeLocalBlockListRecipientCreate"]) {
        $_QLoli = _JJC0E($_QLoli, "domainblocklistmemberedit.php");
        $_QLoli = _JJC0E($_QLoli, "domainblocklistmemberimport.php");
        $_QLoli = _JJC0E($_QLoli, "exportdomainblocklist.php");
      }
  ...
  if(!$_Ii6CO) {
    ...
    $_QLJfI = str_replace('name="Action"', 'name="Action" value="local"', $_QLJfI);
    $_QLJfI = str_replace('domainblocklistmemberedit.php', 'domainblocklistmemberedit.php?action=local&OneMailingListId='.$_POST["OneMailingListId"], $_QLJfI);
  ...
  function _L1RCQ($_Ii6tC, $_QLfol, $_QLoli) {
    ...
    if( isset( $_POST["MembersSearchFor"] ) && ($_POST["MembersSearchFor"] != "") ) {
      ...
      $_Il0o6["Membersfieldname"] = "SearchForDomainname";
    }
    ...
    // Sort
    $_IlJj8 = " ORDER BY Domainname ASC";
    if( isset( $_POST["Memberssortfieldname"] ) && ($_POST["Memberssortfieldname"] != "") ) {
      ...
      $_Il0o6["Memberssortfieldname"] = "SortDomainname";
    ...
    while($_QLO0f=mysql_fetch_array($_QL8i1)) {
      ...
      $_Ql0fO = _L81BJ($_Ql0fO, "<LIST:DOMAIN>", "</LIST:DOMAIN>", $_QLO0f["Domainname"]);
    ...
    if( isset($_POST["MembersSaveFilter"]) ) {
       ...
       include_once("savedoptions.inc.php");
       _JOOFF("BrowseDomainBlocklistsFilter", serialize($_Il0o6) );
    }
