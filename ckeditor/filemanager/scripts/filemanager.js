 ...
 *	@copyright	Authors
 *
 * @many changes and bugfixes 2011 mirko boeer, leipzig
 */
...
var capabilities = new Array('select', 'download', 'rename', 'delete', 'resample');
...
var resampleItem = function(data){
 ...
 var msg = '<div style="font-weight: normal">';
 msg += '<h1>' + lg.changeimagesizeh1 +  '</h1><br />';
 msg += lg.fileName + ': ' + data['Filename'] + '<br />';
 msg += lg.imgcurrentsize + ' ' + data['Properties']['Width'] + 'x' + data['Properties']['Height'] + ' ' + lg.pixel + '<br /><br /><br />';

 msg += '<input type="hidden" name="current_width" id="current_width" value="' + data['Properties']['Width'] + '" />';
 msg += '<input type="hidden" name="current_height" id="current_height" value="' + data['Properties']['Height'] + '" />';

 msg += lg.newimagesizeh1 + ' <br/><br/>';


 msg += '<div><button type="button" id="resize_locked" value="1" style="vertical-align: middle; margin-top: 0px" title=\"' + lg.lockimagedim + '\"><span class="btn_locked"></span></button>';
 msg += lg.width + ': <input id="newwidth" name="newwidth" type="text" value="' + data['Properties']['Width'] + '" size="3" />&nbsp;&nbsp;';
 msg += lg.height + ': <input id="newheight" name="newheight" type="text" value="' + data['Properties']['Height'] + '" size="3" />&nbsp;' + lg.pixel + '&nbsp;</div>';
...
\
      $('#newwidth').change(function(e){ \
        if($('#resize_locked').val() == 0) return true; \
        var v = $('#current_width').val() / $('#current_height').val(); \
        if(v == 0) v = 0.01; \
        $('#newheight').val( Math.round($(this).val() / v) ); \
      }); \
\
      $('#newheight').change(function(e){ \
        if($('#resize_locked').val() == 0) return true; \
        var v = $('#current_width').val() / $('#current_height').val(); \
        if(v == 0) v = 0.01; \
...
\
	...
	var getNewSize = function(v, m){
		if(v != 1) return false;

  var current_width = parseInt(m.find('#current_width').val());
  var current_height = parseInt(m.find('#current_height').val());
  var newwidth = parseInt(m.find('#newwidth').val());
  var newheight = parseInt(m.find('#newheight').val());
 ...
 // M.B.
  ...
  if(thisNode == null) // sub folder?
    ...
    parentNode.find('ul').append(newNode);
		...
		$('#filetree').find('li a[rel="' + path + name + Sep + '"]').click(function(){
...
var getFileInfo = function(file){
 if($('#toolbar') == null) {
    window.stop();
    $("img").removeAttr("src"); //M.B. stop loading images to speed up file selection
 }
	...
	if(browseOnly != true) template += '<button id="resample" name="resample" type="button" value="Resample">' + lg.resizeimage + '</button>';
