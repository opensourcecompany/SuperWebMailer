...
class Net_Socket extends PEAR {
    ...
    var $_streamfunctionsavailable = false;
     ...
     */
    function connect($addr, $port = 0, $persistent = null, $timeout = null, $options = null, $SSLConnection = false)
    {
        ...
        $this->_streamfunctionsavailable = function_exists('stream_context_create') && function_exists('stream_socket_client');

        // M.B.
        if($SSLConnection){
          if(!$options && $this->_streamfunctionsavailable){
             if($timeout == 0) $timeout = 60;
             $options = array(
                      'ssl' => array('SNI_server_name' => $this->addr,
                                     'verify_peer' => false,  // don't verify cert
                                     'verify_host' => false,  // don't verify cert
        ...
        if ($options && function_exists('stream_context_create')) {
            ...
            if ($this->_streamfunctionsavailable) {
    ...
    function setBlocking($mode)
    {
        ...
        if($this->_streamfunctionsavailable)
          ...
          socket_set_blocking($this->fp, $this->blocking); // Alias von stream_set_blocking()
        return true;
    }
    ...
    function setTimeout($seconds = null, $microseconds = null)
    {
        ...
        if ($this->timeout > 0)
           return socket_set_timeout($this->fp, (int) $seconds, (int) $microseconds); // Alias von stream_set_timeout()
    }
    ...
    function getStatus()
    {
       ...
       if($this->_streamfunctionsavailable)
         ...
         return socket_get_status($this->fp);    // socket_get_status — Alias von stream_get_meta_data()
    }
    ...
    {
        if (function_exists("stream_socket_enable_crypto") /*version_compare(phpversion(), "5.1.0", ">=")*/ ) {
            if (!is_resource($this->fp)) {
                return $this->raiseError('not connected');
            }
            ...
            if(!$res){
              // http://php.net/manual/de/function.stream-socket-enable-crypto.php
