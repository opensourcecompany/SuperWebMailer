...
class Net_IMAPProtocol
{
    ...
    function Net_IMAPProtocol($timeout=15)
    {
       self::__construct($timeout);
    }
     ...
     * @since  1.0
     */
     // M.B. $SSLConnection added

    function cmdConnect($host = 'localhost', $port = 143, $SSLConnection = false)
    {
        if ($this->_connected) {
            return new PEAR_Error('already connected, logout first!');
        }
        if (IsPEARError($error = $this->_socket->connect($host,
                                                           $port,
                                                           null,
                                                           $this->_timeout,
        ...
        if (IsPEARError($this->_getRawResponse())) {
    ...
    function _recvLn()
    {
        if (IsPEARError($this->lastline = $this->_socket->gets(8192))) {
            return new PEAR_Error('Failed to write to socket: '
    ...
    {
        $arguments = '';
        while (!IsPEARError($this->_recvLn())) {
    ...
    {
        if (IsPEARError($error = $this->_putCMD($cmdid,
                                                  'AUTHENTICATE',
                                                  'DIGEST-MD5'))) {
            return $error;
        }

        if (IsPEARError($args = $this->_recvLn())) {
            return $args;
        }
        ...
        if (IsPEARError($error = $this->_send("\r\n"))) {
            return $error;
        }
    }
    ...
    function cmdLogout()
    {
        ...
        if (IsPEARError($args = $this->_genericCommand('LOGOUT'))) {
            return $args;
        }
        if (IsPEARError($this->_socket->disconnect())) {
    ...
    function cmdSelect($mailbox)
    {
        ...
        if (!IsPEARError($ret = $this->_genericCommand('SELECT',
                                                         $mailbox_name))) {
    ...
    function cmdStatus($mailbox, $request)
    {
        ...
        if (!IsPEARError($ret) && isset($ret['PARSED'])) {
    ...
    function cmdAppend($mailbox, $msg, $flags_list = '', $time = '')
    {
        ...
        if ($this->hasCapability('LITERAL+') == true) {
            ...
            if (IsPEARError($error = $this->_recvLn())) {
                return $error;
            }

            if (IsPEARError($error = $this->_send($msg . "\r\n"))) {
                return $error;
            }
        }
...
// M.B.
    ...
    {
        if (IsPEARError($res = $this->_genericCommand('STARTTLS'))) {
            return $res;
        }
        ...
        if (IsPEARError($res = $this->cmdCapability())) {
