...
class Net_IMAP extends Net_IMAPProtocol
{
    ...
    function connect($host, $port, $enableSTARTTLS = false, $SSLConnection = False)
    {
        $ret = $this->cmdConnect($host, $port, $SSLConnection);
        if ($ret === true) {
            ...
            if ($this->hasCapability('STARTTLS') === true
                ...
                if (IsPEARError($res = $this->cmdStartTLS())) {
                    return $res;
                }
            }
            return $ret;
    ...
    function login($user, $pass, $useauthenticate = true, $selectMailbox=true)
    {
        if ($useauthenticate) {
            ...
            if (IsPEARError($ret = $this->cmdAuthenticate($user,
                                                            $pass,
                                                            $method))) {
                ...
                if ($this->_serverAuthMethods == null
                    ...
                    if ( IsPEARError($ret = $this->cmdLogin($user, $pass))) {
                        return $ret;
                    }
                } else {
                    return $ret;
        ...
        if ($selectMailbox) {
            //Select INBOX
            if (IsPEARError($ret = $this->cmdSelect($this->getCurrentMailbox()))) {
                return $ret;
            }
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
    ...
    {
        if ($expungeOnExit) {
            if (IsPEARError($ret = $this->cmdExpunge())) {
                return $ret;
            }
            if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
                $ret = $this->cmdLogout();
        ...
        if (IsPEARError($ret = $this->cmdLogout())) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    {
        if (IsPEARError($ret = $this->cmdSelect($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    {
        if (IsPEARError($ret = $this->cmdExamine($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function getParsedHeaders($msg_id,
                              ...
                              $uidFetch = false)
    {
        if (IsPEARError($ret = $this->getRawHeaders($msg_id,
                                                      $part_id,
                                                      $uidFetch))) {
            return $ret;
        }
    ...
    function getMessagesList($msg_id = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdFetch($message_set,
                                                 '(RFC822.SIZE UID)'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
    ...
    function getNumberOfMessages($mailbox = '')
    {
        ...
        if (IsPEARError($ret = $this->cmdStatus($mailbox, 'MESSAGES'))) {
            return $ret;
        }
    ...
    function getNumberOfUnSeenMessages($mailbox = '')
    {
        ...
        if (IsPEARError($ret = $this->cmdStatus($mailbox, 'UNSEEN'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret["RESPONSE"]["CODE"]
    ...
    function getNumberOfRecentMessages($mailbox = '')
    {
        ...
        if (IsPEARError($ret = $this->cmdStatus($mailbox, 'RECENT'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function getStatus($mailbox = '')
    {
        ...
        if (IsPEARError($ret = $this->cmdStatus($mailbox, array('MESSAGES', 'RECENT', 'UIDNEXT', 'UIDVALIDITY', 'UNSEEN')))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function getMailboxSize($mailbox = '')
    {
        ...
        if ($mailbox != '' && $mailbox != $this->getCurrentMailbox()) {
            ...
            if (IsPEARError($ret = $this->selectMailbox($mailbox))) {
                return $ret;
            }
        }
        ...
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            ...
            if ($mailbox != '' && $mailbox != $this->getCurrentMailbox()) {
                if (IsPEARError($ret = $this->selectMailbox($mailbox_aux))) {
                        return $ret;
                }
            }
                          ...
                          $uidCopy = false)
    {
        if ($source_mailbox == null) {
            ...
            if (IsPEARError($ret = $this->selectMailbox($source_mailbox))) {
                return $ret;
            }
        }
    ...
    {
        if (IsPEARError($ret = $this->cmdNamespace())) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    ******************************************************************/
    ...
    function getHierarchyDelimiter($mailbox = '')
    {
        /* RFC2060 says:
            "the command LIST "" "" means get the hierachy delimiter:
             ...
             exist."
        */
        if (IsPEARError($ret = $this->cmdList($mailbox, ''))) {
            return $ret;
        }
                          ...
                          $returnAttributes = false)
    {
        ...
        if (IsPEARError($ret = $this->cmdList($reference, $mailbox))) {
            return $ret;
        }
    ...
    function mailboxExist($mailbox)
    {
        ...
        if (IsPEARError($ret = $this->getMailboxes($mailbox, true))) {
            return $ret;
        }
        if (count($ret) > 0) {
    ...
    {
        if (IsPEARError($ret = $this->cmdCreate($mailbox, $options))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function deleteMailbox($mailbox)
    {
        ...
        if (IsPEARError($ret = $this->cmdDelete($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    {
        if (IsPEARError($ret = $this->cmdRename($oldmailbox,
                                                  $newmailbox,
                                                  $options))) {
            return $ret;
        }
    ...
    ******************************************************************/
    ...
    function subscribeMailbox($mailbox = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdSubscribe($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function unsubscribeMailbox($mailbox = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdUnsubscribe($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
                                     ...
                                     $returnAttributes = false)
    {
        ...
        if (IsPEARError($ret = $this->cmdLsub($reference, $mailbox))) {
            return $ret;
        }
    ...
    ******************************************************************/
    ...
    {
        if (IsPEARError($resp = $this->getFlags($message_nro))) {
            return $resp;
        }
        if (isset($resp[0])) {
            if (is_array($resp[0])) {
    ...
    ******************************************************************/
    ...
    function getStorageQuotaRoot($mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdGetQuotaRoot($mailbox_name))) {
    ...
    function getStorageQuota($mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdGetQuota($mailbox_name))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    {
        if (IsPEARError($ret = $this->cmdSetQuota($mailbox_name, $quota))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    {
        if (IsPEARError($ret = $this->cmdSetQuota($mailbox_name,
                                                    '',
                                                    $quota))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    ******************************************************************/
    ...
    function getACL($mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdGetACL($mailbox_name))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    {
        if (IsPEARError($ret = $this->cmdSetACL($mailbox_name, $user, $acl))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    {
        if (IsPEARError($ret = $this->cmdDeleteACL($mailbox_name, $user))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function getMyRights($mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdMyRights($mailbox_name))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    function getACLRights($user,$mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdListRights($mailbox_name, $user))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE']
    ...
    ******************************************************************/
    ...
    function setAnnotation($entry, $values, $mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdSetAnnotation($mailbox_name,
                                                         $entry,
                                                         $values))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    function deleteAnnotation($entry, $values, $mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdDeleteAnnotation($mailbox_name,
                                                            $entry,
                                                            $values))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    function getAnnotation($entries, $values, $mailbox_name = null)
    {
        ...
        if (IsPEARError($ret = $this->cmdGetAnnotation($mailbox_name,
                                                         $entries,
                                                         $values))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    *********************************************************/
    ...
    {
        if (IsPEARError($ret = $this->getMessages($msg_id, false))) {
            return $ret;
        }
