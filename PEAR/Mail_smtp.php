...
class Mail_smtp extends Mail {
    ...
    function __construct($params)
    {
        ...
        if (isset($params['debugfilename'])) $this->debugfilename = $params['debugfilename'];
    ...
    function send($recipients, $headers, $body)
    {
        ...
        if (is_object($this->_smtp) === false) {
            $this->_smtp = new Net_SMTP($this->host, $this->port,
                                        $this->localhost, $this->pipelining, $this->SSLConnection);

            /* If we still don't have an SMTP object at this point, fail. */
            if (is_object($this->_smtp) === false) {
                return PEARraiseError('Failed to create a Net_SMTP object',
            ...
            if (is_a($res, 'PEAR_Error')) {
                $error = $this->_error('Failed to connect to ' .
                                       $this->host . ':' . $this->port,
                                       $res);
                return PEARraiseError($error, PEAR_MAIL_SMTP_ERROR_CONNECT);
            }
            ...
            if ($this->auth) {
                ...
                if (is_a($res, 'PEAR_Error')) {
                    ...
                    $this->_smtp->rset();
                    return PEARraiseError($error, PEAR_MAIL_SMTP_ERROR_AUTH);
                }
            }
        }
        ...
        if (!isset($from)) {
            $this->_smtp->rset();
            return PEARraiseError('No From: address has been provided',
    ...
    function disconnect()
    {
        ...
        if (is_object($this->_smtp)) {
            $this->_smtp->disconnect(); // M.B. ever disconnect
            $this->_smtp = null;
        }
