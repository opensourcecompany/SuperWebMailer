...
class Net_DNS_Question
{
    ...
    function __construct($qname, $qtype, $qclass)
    {
        ...
        if ( ( is_null($this->Net_DNS->typesbyname($qtype)) ||
               is_null($this->Net_DNS->classesbyname($qtype)) )
          && !is_null($this->Net_DNS->classesbyname($qclass))
          && !is_null($this->Net_DNS->typesbyname($qclass)))
        {
    ...
    function Net_DNS_Question($qname, $qtype, $qclass){
      self::__construct($qname, $qtype, $qclass);
    }
    ...
    function data($packet, $offset)
    {
        ...
        $data .= pack('n', $this->Net_DNS->typesbyname(strtoupper($this->qtype)));
        $data .= pack('n', $this->Net_DNS->classesbyname(strtoupper($this->qclass)));
        return $data;
    }
