...
class Net_DNS_Header
{
    ...
    {
        $this->Net_DNS_Resolver = new Net_DNS_Resolver();
        $this->Net_DNS = new Net_DNS();
        ...
        if ($data != '') {
            ...
            $this->id      = $this->Net_DNS_Resolver->nextid();
            $this->qr      = 0;
            $this->opcode  = 0;
            $this->aa      = 0;
            $this->tc      = 0;
        ...
        if ($this->Net_DNS->opcodesbyval($this->opcode)) {
            $this->opcode = $this->Net_DNS->opcodesbyval($this->opcode);
        }
        if ($this->Net_DNS->rcodesbyval($this->rcode)) {
            $this->rcode = $this->Net_DNS->rcodesbyval($this->rcode);
        }
    }
    ...
    function data()
    {
        ...
        $opcode = $this->Net_DNS->opcodesbyname($this->opcode);
        $rcode  = $this->Net_DNS->rcodesbyname($this->rcode);
