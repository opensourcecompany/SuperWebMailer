 ...
 */
 ...
 /*
  ...
  M.B. bugfix for switching wysiwyg mode to source mode, events triggered more than once => remove resizer set to null and recreate it
  M.B. Extended to ever resize with Aspect Ratio in all browsers, except in IE
...
(function() {
  ...
  var isWebkit = (!CKEDITOR.env.ie || CKEDITOR.env.ie && CKEDITOR.env.edge);//CKEDITOR.env.webkit || CKEDITOR.env.edge; //('WebkitAppearance' in document.documentElement.style);
  ...
  CKEDITOR.plugins.add(PLUGIN_NAME, {
    ...
    init: function(editor) {
      if (!isWebkit || !editor.config.disableObjectResizing) { // in config disableObjectResizing must be true
        return;
      }
      //onDomReady handler
  ...
  function init(editor) {
    ...
    editor.on('afterUndoImage', function(event) {
    ...
    editor.on('beforeModeUnload', function(event) {
      ...
      event.removeListener('beforeModeUnload');
      resizer.hide();
      resizer = null;
    });
    ...
    var resizeTimeout;
    editor.window.on('resize', function(event) {
      if(resizer == null) return;
  ...
  Resizer.prototype = {
    ...
    getNatural: function( image ) {
     ...
     dimensions["RatioForLeftRight"] = dimensions.height / dimensions.width;
     dimensions["RatioForOthers"] = dimensions.width / dimensions.height;
  			return dimensions;
  		},

    show: function(el) {
      this.dimensions = this.getNatural(el);
      this.el = el;
    ...
    initDrag: function(e) {
      ...
      drag.onComplete = function() {
        ...
        var data = {element: resizer.el, old_dimensions: resizer.dimensions, new_dimensions: {width: resizer.el.width,	height: resizer.el.height} };
        resizer.editor.fire('imagesizechanged', data);
      };
      drag.start(e);
    },
    ...
    calculateSize: function(data) {
      ...
      // M.B.
      if(!data.keys.shift){
        if( attr.indexOf('lm') > -1  || attr.indexOf('rm') > -1 ){
           box.height = Math.trunc(this.dimensions.RatioForLeftRight * box.width);
        }else{
           box.width = Math.trunc(this.dimensions.RatioForOthers * box.height);
        }
      }
